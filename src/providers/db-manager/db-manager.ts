...
export class DbManagerProvider {
    ...
    initTables(db){
        ...
        db.executeSql('CREATE TABLE IF NOT EXISTS CASES (ID_CASE INTEGER, ID_ELEMENT INTEGER, ID_ELEMENT_TYPE INTEGER, ID_CASE_STATUS INTEGER, NR_CASE INTEGER, PRIMARY KEY(ID_CASE) )', []);
        db.executeSql('CREATE TABLE IF NOT EXISTS COMBO_CATEGORY (ID_COMBO_CATEGORY INTEGER, NM_COMBO_CATEGORY TEXT )', []);
        db.executeSql('CREATE TABLE IF NOT EXISTS ETYPE_CONFIG_ATTRIBUTE (ID_ATTRIBUTE INTEGER, ID_ELEMENT_TYPE_CONFIG INTEGER, FG_MANDATORY INTEGER, PRIMARY KEY(ID_ATTRIBUTE,ID_ELEMENT_TYPE_CONFIG) )', []);
        db.executeSql('CREATE TABLE IF NOT EXISTS WORKITEM_ELEMENT (ID_WORK_ITEM_ELEMENT INTEGER, ID_ELEMENT INTEGER, ID_ELEMENT_TYPE_CONFIG INTEGER, ID_CASE INTEGER, ID_WORK_ITEM_STATUS INTEGER, DE_CANCEL TEXT, DE_NOTES TEXT, PRIMARY KEY(ID_WORK_ITEM_ELEMENT) )', []);
